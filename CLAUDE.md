...
3. **Migrate secrets**: Extract values from existing `secret.sops.yaml` and add to Infisical first
4. **Add secrets** via CLI: Create folders first, then `infisical secrets set KEY=value
   --path="/apps/namespace/app"`
5. **Remove SOPS** references: Delete `secret.sops.yaml` from resources AFTER migration
6. **Validate**: Check `kubectl get secret appname-secret -o yaml` for synced data
