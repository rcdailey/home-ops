...
**WORKFLOW:** User handles authentication/initialization. Claude manages secrets via CLI.

**Claude Responsibilities:**

- `infisical secrets set KEY=value` - Add/update secrets
- `infisical secrets get KEY` - Retrieve secrets
- `infisical secrets delete KEY` - Remove secrets
- Use `--path="/apps/${NAMESPACE}/${APP}"` for app-specific secrets
- Use `--plain --silent` for script-friendly output
- **CRITICAL**: Create folder structure BEFORE setting secrets (CLI fails silently if path doesn't
  exist)

**User Responsibilities:**

- `infisical login` and `infisical init` setup
...
```bash
# Create folder structure first (REQUIRED)
infisical secrets folders create --name apps --path="/"
infisical secrets folders create --name default --path="/apps"
infisical secrets folders create --name silverbullet --path="/apps/default"

# Set secrets for app (use standard env var names, no prefixes needed)
infisical secrets set DATABASE_URL=postgres://user:pass@host:5432/db --path="/apps/default/silverbullet"
infisical secrets set API_KEY=your-api-key --path="/apps/default/silverbullet"

# Get secret value
infisical secrets get API_KEY --path="/apps/default/silverbullet" --plain --silent

# List all secrets in path
infisical secrets --path="/apps/default/silverbullet"
```

**Key Flags:**

- `--path`: Organize secrets by app path
- `--plain`: Unformatted output for scripts
- `--silent`: Suppress informational messages
- `--env`: Environment (defaults to "prod" via .infisical.json)

**Secret Key Naming:**

- Use standard environment variable names (`DATABASE_URL`, `API_KEY`, `SECRET_KEY`)
- NO prefixes needed (path provides app isolation)
- Follow application's expected variable names

**Secret Organization:**

- **Project**: Single "home-ops" project for all cluster secrets
- **Environment**: "prod" for production cluster
- **Path Structure**: `/apps/${NAMESPACE}/${APP}` (e.g., `/apps/default/silverbullet`)
- **Component Architecture**: Kustomize component at `kubernetes/components/infisical/`
- **Migration Pattern**: Replace SOPS files with Infisical component inclusion

**Adding Secrets to Apps:**

1. **Add component** to `kustomization.yaml`: `- ../../../components/infisical`
2. **Set variables** in `ks.yaml` under `postBuild.substitute`:
...
3. **MANDATORY: Add auto-reload annotation** to controller in `helmrelease.yaml`:

   ```yaml
   controllers:
     main:  # or whatever controller name
       annotations:
         secrets.infisical.com/auto-reload: "true"
   ```

4. **Migrate secrets**: Extract values from existing `secret.sops.yaml` and add to Infisical first
5. **Add secrets** via CLI: Create folders first, then `infisical secrets set KEY=value
   --path="/apps/namespace/app"`
6. **Remove SOPS** references: Delete `secret.sops.yaml` from resources AFTER migration
7. **Validate**: Check `kubectl get secret appname-secret -o yaml` for synced data
