...
4. Consider production cutover to torrent.domain.example after stability validation
...
**Deployment Components**: bjw-s app-template chart, default namespace, ProtonVPN WireGuard provider with manual key configuration, automatic port forwarding via NAT-PMP, simplified DNS architecture (no dnsdist), torrent-test.domain.example initial subdomain.
...
- Domain: domain.example, Test Domain: *.test.domain.example
- Cloudflare Tunnel ID: 6b689c5b-81a9-468e-9019-5892b3390500
- Tunnel Target: external.domain.example → 192.168.1.73
...
Authentik Integration: Created dedicated proxy provider for qBittorrent with domain-level forward auth mode, external host https://torrent-test.domain.example, internal host http://qbittorrent.default.svc.cluster.local:8080. Configured embedded outpost with authentik_host set to https://auth-test.domain.example and assigned qBittorrent application.
...
Validation Results: Forward authentication operational with proper OAuth flow redirecting unauthenticated users to auth-test.domain.example login. Post-authentication access to torrent-test.domain.example succeeds with user context headers forwarded to qBittorrent. VPN connectivity maintained through ProtonVPN WireGuard with port forwarding active.
...
Authentik Integration Strategy: Current setup uses auth-test.domain.example subdomain for testing, production auth.domain.example planned. Outpost configuration required for /outpost.goauthentik.io/auth forward auth endpoint. Application and provider setup needed in Authentik for domain-level protection. Policy configuration determines access control for qBittorrent.
...
Subdomain Correction: Updated from torrent.domain.example to torrent-test.domain.example for initial deployment testing, matching auth-test pattern. Production cutover to torrent.domain.example after validation. External gateway usage confirmed (192.168.1.73) for public-facing services requiring forward authentication.

### 2025-07-12 - Cloudflare Tunnel Fix and External Access Restoration

Successfully resolved external access failure for home.domain.example by diagnosing and fixing Cloudflare tunnel configuration after Envoy Gateway migration. Root cause was outdated service reference preventing tunnel from reaching new gateway infrastructure.

Tunnel Issue Diagnosis: External DNS correctly resolved home.domain.example to Cloudflare IPs but resulted in "Bad gateway" 502 errors. Investigation revealed tunnel config still referenced obsolete cilium-gateway-external.kube-system.svc.cluster.local service which no longer existed after Envoy migration. Tunnel logs showed DNS lookup failures preventing connection to backend services.
...
Successfully completed Authentik deployment with systematic authentication troubleshooting. Resolved PostgreSQL user password initialization and Redis authentication configuration. All components operational with HTTPRoute active at auth-test.domain.example and admin credentials verified working.
