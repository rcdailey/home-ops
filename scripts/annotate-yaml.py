...
def extract_resource_from_schema(
    filename: str, schema_data: dict
    ...
    try:
        ...
        for json_file in json_files:
            try:
                ...
                if (
                    schema_data.get("kind") == "CustomResourceDefinition"
                    and "spec" in schema_data
                ):
...
def add_fluxcd_schemas(mapping: Dict[str, str]):
    ...
    try:
        ...
        # Fetch schema file listing from GitHub API
        try:
            curl_result = subprocess.run(
                [
                    "curl",
                    "-s",
                    "https://api.github.com/repos/fluxcd-community/flux2-schemas/contents",
                ],
                capture_output=True,
                text=True,
                timeout=30,
...
def add_datree_schemas(mapping: Dict[str, str]):
    ...
    try:
        filepath = download_cached_file(
            DATREE_BASE_URL + "index.yaml", "datree_index.yaml"
        )
...
def annotate_file(
    ...
    for doc in raw_docs:
        ...
        schema_lines = [
            ln
            for ln in lines
            if ln.strip().startswith("# yaml-language-server: $schema")
        ]
        if schema_lines:
        ...
        if api_version and kind:
            schema_url = find_schema_url(
                api_version, kind, schema_mapping, verbose=False
            )
            if schema_url:
                # Handle local schemas - convert to relative path from current file
                if schema_url.startswith("LOCAL_SCHEMA:"):
                    absolute_schema_path = schema_url[
                        13:
                    ]  # Remove LOCAL_SCHEMA: prefix
                    final_schema_url = get_relative_path_from_file(
                        file_path, absolute_schema_path
                    )
                else:
                ...
                if lines and lines[0].strip() == "---":
                    lines.insert(
                        1, f"# yaml-language-server: $schema={final_schema_url}"
                    )
                else:
                    # Insert --- first, then schema comment
                    lines.insert(
                        0, f"# yaml-language-server: $schema={final_schema_url}"
                    )
                    lines.insert(0, "---")
    ...
    if dry_run:
        if single_file_mode and has_changes:
            ...
            for kind, api_version, schema_url in schema_info:
                print(
                    f"  - {Colors.YELLOW}{kind}{Colors.RESET}/{Colors.CYAN}{api_version}{Colors.RESET} :: {Colors.MAGENTA}{schema_url}{Colors.RESET}"
                )
    else:
        if has_changes:
...
def main():
    parser = argparse.ArgumentParser(
        description="Add YAML Language Server schema annotations to Kubernetes YAML files"
    )
    parser.add_argument(
        "paths",
        nargs="*",
        default=["kubernetes"],
        help="YAML files or directories to process (directories are searched recursively). Defaults to 'kubernetes'",
    )
    parser.add_argument(
        "-n",
        "--dry-run",
    ...
    if missing_schemas:
        ...
        print(
            f"The following {len(missing_schemas)} resource types could not be mapped to schemas:"
        )
        print()
        ...
        print("check if newer versions are available in the schema catalogs.")
