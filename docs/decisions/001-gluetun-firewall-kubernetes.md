...
The qbittorrent application uses gluetun as a VPN sidecar to route torrent traffic through
ProtonVPN. The firewall was disabled (`FIREWALL: "off"`) as a workaround for Kubernetes networking
issues, leaving no kill switch protection - if the VPN disconnects, traffic leaks unprotected to the
